...
class WorkerDetailViewTest(TestCase):
    ...
    def test_worker_detail_view(self):
        response = self.client.get(reverse('catalog:worker-detail', kwargs={'pk': self.worker.pk}))
        self.assertEqual(response.status_code, 302)  # Should redirect to login if not authenticated
        self.client.login(username='testuser', password='testpassword')
        response = self.client.get(reverse('catalog:worker-detail', kwargs={'pk': self.worker.pk}))
        self.assertEqual(response.status_code, 200)
...
class WorkerUpdateViewTest(TestCase):
    ...
    def test_worker_update_view(self):
        self.client.login(username='testuser', password='testpassword')
        response = self.client.get(reverse('catalog:worker-update', kwargs={'pk': self.worker.pk}))
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'catalog/worker_form.html')
...
class WorkerDeleteViewTest(TestCase):
    ...
    def test_worker_delete_view(self):
        self.client.login(username='testuser', password='testpassword')
        response = self.client.get(reverse('catalog:worker-delete', kwargs={'pk': self.worker.pk}))
        self.assertEqual(response.status_code, 200)
...
class TaskDetailViewTest(TestCase):
    ...
    def test_task_detail_view(self):
        response = self.client.get(reverse('catalog:task-detail', kwargs={'pk': self.task.pk}))
        self.assertEqual(response.status_code, 302)  # Should redirect to login if not authenticated
        self.client.login(username='testuser', password='testpassword')
        response = self.client.get(reverse('catalog:task-detail', kwargs={'pk': self.task.pk}))
        self.assertEqual(response.status_code, 200)
...
class TaskUpdateViewTest(TestCase):
    ...
    def test_task_update_view(self):
        self.client.login(username='testuser', password='testpassword')
        response = self.client.get(reverse('catalog:task-update', kwargs={'pk': self.task.pk}))
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'catalog/task_form.html')
...
class TaskDeleteViewTest(TestCase):
    ...
    def test_task_delete_view(self):
        self.client.login(username='testuser', password='testpassword')
        response = self.client.get(reverse('catalog:task-delete', kwargs={'pk': self.task.pk}))
        self.assertEqual(response.status_code, 200)
